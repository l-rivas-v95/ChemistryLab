...
<text x='210' y='165' text-anchor='middle' font-family='Arial' font-size='13' fill='#7f1d1d'>{html.escape(message[:90])}</text>
</svg>"""
...
def main() -> int:
    ...
    try:
        ...
        for name, _ in MOLECULES:
            (out_dir / f"{slug(name)}.svg").write_text(error_svg("Indigo missing", str(exc)), encoding="utf-8")
        return 2
    ...
    for name, smiles in MOLECULES:
        ...
        try:
            ...
            output.write_text(error_svg("Indigo error", str(exc)), encoding="utf-8")
